...
wxString GRIBTable::GetCompRefl(GribRecord **recordarray)
{
    ...
    if( recordarray[Idx_COMP_REFL] ) {
        double cape = recordarray[Idx_COMP_REFL]->
            getInterpolatedValue(m_cursor_lon, m_cursor_lat, true );

        if( cape != GRIB_NOTDEF ) {
            cape = m_pGDialog->m_OverlaySettings.CalibrateValue(GribOverlaySettings::COMP_REFL, cape);
            skn.Printf( wxString::Format( _T("%5.0f ") + m_pGDialog->m_OverlaySettings.GetUnitSymbol(GribOverlaySettings::COMP_REFL), cape ) );
            m_pDataCellsColour = m_pGDialog->pPlugIn->m_pGRIBOverlayFactory->GetGraphicColor(GribOverlaySettings::COMP_REFL, cape);
        }
    }
    return skn;
}

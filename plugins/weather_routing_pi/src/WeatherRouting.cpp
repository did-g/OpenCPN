...
void WeatherRouting::Export(RouteMapOverlay &routemapoverlay)
{
    ...
    for(auto const &it : plotdata) {
        ...
        if (!std::isnan(speed)) newPoint->m_MarkDescription.Printf("VMG=%.15f", speed);
        speed = it.VBG;
    ...
    if (p) {
        ...
        // XXX currently last speed computed in getplotdata is likely wrong, it
        // doesn't use the right duration
        if (0 && !std::isnan(speed)) newPoint->m_MarkDescription.Printf("VMG=%.15f", speed);
        // shared endpoint ?

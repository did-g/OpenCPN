...
static int CompareScales( int *i1, int *i2 )
{
    ...
    if( charts_scale_eq(cte1.GetScale(), cte2.GetScale()) ) {  // same scales, so sort on dbIndex
        float lat1, lat2;
        lat1 = cte1.GetLatMax();
        lat2 = cte2.GetLatMax();
...
ArrayOfInts Quilt::GetCandidatedbIndexArray( bool from_ref_chart, bool exclude_user_hidden )
{
    ...
    for( unsigned int ir = 0; ir < m_pcandidate_array->GetCount(); ir++ ) {
        ...
        {
            if( charts_scale_ge ( pqc->ChartScale, m_reference_scale) ) {
...
int Quilt::GetNewRefChart( void )
{
    ...
    if( im > 0 ) {
        for( unsigned int is = 0; is < im; is++ ) {
            ...
            if( ( charts_scale_ge (m.GetScale(), m_reference_scale) )
...
//             }
    ...
    if( b_fullscreen ) {
        ...
        for( int i = 0; i < n_all_charts; i++ ) {
            ...
            if( !charts_scale_ge (candidate_chart_scale, reference_scale) ) {
            ...
            if( ( charts_scale_ge (candidate_chart_scale, reference_scale) ) || ( zoom_factor > .2 ) ) {
                bool b_add = true;
...
//    vp_local.SetRotationAngle( 0. );
    ...
    if( !vp_region.Empty() ) {
        for( ir = 0; ir < m_pcandidate_array->GetCount(); ir++ ) {
            ...
            if( charts_scale_ge (cte.GetScale() , m_reference_scale)  ) {
    ...
    if( !b_vis && m_pcandidate_array->GetCount() ) {
        ...
        for( int i = m_pcandidate_array->GetCount() - 1; i >= 0; i-- ) {
            ...
            if( !vpck_region.Empty() ) {
                if( add_scale ) {
                    if( charts_scale_eq(add_scale, cte.GetScale()) ) 
                        pqc->b_include = true;
                    ;
                } else {
                    pqc->b_include = true;
